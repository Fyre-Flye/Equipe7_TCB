...
const RUNS: usize = 30;
const SIZES: [(&str, usize); 3] = [("pequena", 1_000), ("media", 10_000), ("grande", 1_000_000)];
const CASES: [&str; 3] = ["melhor", "medio", "pior"];
...
fn generate_vector(case_name: &str, size: usize, run: usize) -> Vec<usize> {
    match case_name {
        "melhor" => (0..size).collect(),
        "pior" => (0..size).rev().collect(),
        "medio" => {
            let mut seed = 42 + size as u64 + run as u64;
            let mut values = Vec::with_capacity(size);

            for _ in 0..size {
                seed = next_lcg(seed);
                values.push((seed % size as u64) as usize);
            }
...
fn run_benchmark() -> std::io::Result<()> {
    ...
    writeln!(
        writer,
        "linguagem,caso,tamanho_label,n,k,execucao,tempo_segundos,ordenado"
    )?;

    for (size_label, size) in SIZES {
        for case_name in CASES {
            for run in 1..=RUNS {
                let base_values = generate_vector(case_name, size, run);
